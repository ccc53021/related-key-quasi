...
def get_sr_table_by_n(bit_n):
    tt = [15, 12, 13, 14, 10, 9, 8, 11, 6, 5, 4, 7, 1, 2, 3, 0]
    table = []
    for ii in tt:
...
def get_matrix_t_table_by_n(bit_n):
    table = []
    for i in range(8 * n):
        one_bit_table = [i]
        table.append(one_bit_table)
    for i in range(8 * n, 12 * n):
        one_bit_table = [i - 8 * n, i]
        table.append(one_bit_table)
    for i in range(12 * n, 16 * n):
        one_bit_table = [i - 12 * n, i - 8 * n, i]
        table.append(one_bit_table)
...
def get_quasidifferentials_by_one_difference_route(difference_route, route_number, min_weight, max_weight):
    def search_quasidifferential(one_difference_characteristic, f):
        ...
        for i in range(differential_characteristic_rounds):
            permute_bits_sr(mask_a[i], mask_b[i])
            for j in range(0, state_bits, sbox_bits):
                u = mask_b[i][j + sbox_bits - 1:j]
                v = mask_c[i][j + sbox_bits - 1:j]
                a = (one_difference_characteristic[4 * i + 2] >> j) & (2 ** sbox_bits - 1)
                b = (one_difference_characteristic[4 * i + 3] >> j) & (2 ** sbox_bits - 1)
        ...
        def compute_sign(diffs, trail):
            ...
            for round in range(differential_characteristic_rounds):
                one_round_difference_input = diffs[4 * round + 2]
                one_round_difference_s = diffs[4 * round + 3]
                one_round_mask_input = trail[3 * round + 1]
                one_round_mask_s = trail[3 * round + 2]
...
import differential_p_30 as differentials
