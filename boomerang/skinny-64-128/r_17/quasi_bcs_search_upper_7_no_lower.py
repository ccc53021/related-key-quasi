...
def get_quasidifferentials_by_one_fixed_input_and_output(diff_route, route_number, min_weight, max_weight):
    def search_differential(f):
        ...
        # difference
        # input
        u0_a = [btor.Var(btor.BitVecSort(state_bits), "u0_a%d" % i) for i in range(differential_characteristic_rounds_upper + 1)]
        u1_a = [btor.Var(btor.BitVecSort(state_bits), "u1_a%d" % i) for i in range(differential_characteristic_rounds_upper + 1)]
        # after sb
        u0_b = [btor.Var(btor.BitVecSort(state_bits), "u0_b%d" % i) for i in range(differential_characteristic_rounds_upper)]
        u1_b = [btor.Var(btor.BitVecSort(state_bits), "u1_b%d" % i) for i in range(differential_characteristic_rounds_upper)]
        # after_k
        u0_c = [btor.Var(btor.BitVecSort(state_bits), "u0_c%d" % i) for i in range(differential_characteristic_rounds_upper)]
        u1_c = [btor.Var(btor.BitVecSort(state_bits), "u1_c%d" % i) for i in range(differential_characteristic_rounds_upper)]
        u0_d = [btor.Var(btor.BitVecSort(state_bits), "u0_d%d" % i) for i in range(differential_characteristic_rounds_upper)]
        u1_d = [btor.Var(btor.BitVecSort(state_bits), "u1_d%d" % i) for i in range(differential_characteristic_rounds_upper)]
        ...
        for target in range(min_weight, max_weight):
            ...
            while True:
                ...
                if r == btor.SAT:
                    ...
                    u0_a_final = int(u0_a[differential_characteristic_rounds_upper].assignment, base=2)
                    u1_a_final = int(u1_a[differential_characteristic_rounds_upper].assignment, base=2)
                    print("", file=f)
                    print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(u0_a_final, u1_a_final, diff_route[4 * (differential_characteristic_rounds_upper)]), file=f)
                    ...
                    print("    ],", file=f)
                    previous.append((s_route, [u0_a[i].assignment for i in range(differential_characteristic_rounds_upper + 1)], [u1_a[i].assignment for i in range(differential_characteristic_rounds_upper + 1)]))
                else:
    ...
    filename = "result/quasi_bcs_search_upper_{}_no_lower_w_{}_to_{}.txt".format(differential_characteristic_rounds_upper, min_weight, max_weight)
    f = open(filename, "w")
...
diff_route = [
    0x1000000000000000,  # input
    0x8000000000000000,  # after sb
    0x8000000000000000,  # after tk1
    0x0800000000000000,  # after sr

    0x0000000000000800,  # after mc
    0x0000000000000500,  # after sb
    0x0000000000000000,  # after tk2
    0x0000000000000000,  # after sr

    0x0000000000000000,  # after mc
    0x0000000000000000,  # after sb
    0x0000000000000000,  # after tk3
    0x0000000000000000,  # after sr

    0x0000000000000000,  # after mc
    0x0000000000000000,  # after sb
    0x0000000000000000,  # after tk4
    0x0000000000000000,  # after sr
    ...
    0x0000000000000000,  # after mc
    0x0000000000000000,  # after sb
    0x000000000b000000,  # after tk6
    0x00000000b0000000,  # after sr

    0x0000b00000000000,  # after mc
    0x0000100000000000,  # after sb
    0x0000100000000000,  # after tk7
    0x0000001000000000,  # after sr
