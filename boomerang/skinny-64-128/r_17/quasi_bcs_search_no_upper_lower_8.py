...
def get_quasidifferentials_by_one_fixed_input_and_output(diff_route, route_number, min_weight, max_weight):
    def search_differential(f):
        ...
        # difference
        # input
        u0_a = [btor.Var(btor.BitVecSort(state_bits), "u0_a%d" % i) for i in range(differential_characteristic_rounds_lower + 1)]
        u1_a = [btor.Var(btor.BitVecSort(state_bits), "u1_a%d" % i) for i in range(differential_characteristic_rounds_lower + 1)]
        # after sb
        u0_b = [btor.Var(btor.BitVecSort(state_bits), "u0_b%d" % i) for i in range(differential_characteristic_rounds_lower)]
        u1_b = [btor.Var(btor.BitVecSort(state_bits), "u1_b%d" % i) for i in range(differential_characteristic_rounds_lower)]
        # after_k
        u0_c = [btor.Var(btor.BitVecSort(state_bits), "u0_c%d" % i) for i in range(differential_characteristic_rounds_lower)]
        u1_c = [btor.Var(btor.BitVecSort(state_bits), "u1_c%d" % i) for i in range(differential_characteristic_rounds_lower)]
        # after sr
        u0_d = [btor.Var(btor.BitVecSort(state_bits), "u0_d%d" % i) for i in range(differential_characteristic_rounds_lower)]
        ...
        for i in range(differential_characteristic_rounds_lower):
            for j in range(0, state_bits, sbox_bits):
                ...
                small_quasi_ddt_weight_lists_w_0_to_7 = get_small_quasi_tables_weight_lists_by_u_and_v(small_quasi_ddt,
                                                                                                       1)
        ...
        set_fixed_difference(u0_a[differential_characteristic_rounds_lower], 0x0)
        ...
        for target in range(min_weight, max_weight):
            ...
            while True:
                ...
                if r == btor.SAT:
                    ...
                    u0_a_final = int(u0_a[differential_characteristic_rounds_lower].assignment, base=2)
                    u1_a_final = int(u1_a[differential_characteristic_rounds_lower].assignment, base=2)
                    print("", file=f)
                    print("     [0x{:016x}, 0x{:016x}, 0x{:016x}],".format(u0_a_final, u1_a_final, diff_route[4 * (differential_characteristic_rounds_lower)]), file=f)
                    ...
                    print("    ],", file=f)
                    previous.append((s_route, [u0_a[i].assignment for i in range(differential_characteristic_rounds_lower + 1)], [u1_a[i].assignment for i in range(differential_characteristic_rounds_lower + 1)]))
                else:
    ...
    filename = "result/new_quasi_ddt_and_bct_64_128_fixed_route_no_upper_lower_{}_w_{}_to_{}.txt".format(differential_characteristic_rounds_lower, min_weight, max_weight)
    f = open(filename, "w")
...
diff_route = [
    ...
    0x0c0000000d000000,  # after sb
    0x0c00000004000000,  # after tk2
    0x00c0000040000000,  # after sr

    0x00004000000000c0,  # after mc
    0x0000200000000020,  # after sb
    0x0000200000020020,  # after tk3
    0x0000002000200020,  # after sr

    0x0000000000200000,  # after mc
    0x0000000000100000,  # after sb
    ...
    0x0000000000000000,  # after mc
    0x0000000000000000,  # after sb
    0x0000000000400000,  # after tk7
    0x0000000004000000,  # after sr

    0x0000040000000000,  # after mc
    0x0000020000000000,  # after sb
    0x0000020020000000,  # after tk8
    0x0000000200020000,  # after sr

    0x0002000000000002,  # after mc
]
